...
class MongoModel extends EloquentModel
{
    ...
    public function scopeSelect($query, $selection = null)
    {
        ...
        foreach ($selection as $key => $value) {
            if ($value instanceof Expression) {
                ...
                if (method_exists(DB::connection('mongodb'), 'getQueryGrammar')) {
                    $qgrammar =  DB::connection('mongodb')->getQueryGrammar();
        ...
        foreach ($shelper as $find => $search_property) {            
            if (array_key_exists($find, $_GET) || array_key_exists($find . '>', $_GET) || array_key_exists($find . '<', $_GET)) {
                ...
                if ($search_property instanceof Expression) {
                    if (method_exists(DB::connection('mongodb'), 'getQueryGrammar')) {
                        $qexpressionValue = $search_property->getValue(DB::connection('mongodb')->getQueryGrammar());
                    } else {
                    ...
                    // A relációt törölni kell, mert csak úgy működik a keresés
                    $search_property = $prop;

                    $rel = implode('.', $tmp);
                    $type = $basemodel->getSearch($prop) ?: 'integer';
                    ...
                    case 'integer':
                        if (array_key_exists($find, $_GET)) {
                            ...
                            if (preg_match('/,/', $value)) {
                                ...
                                $numberArray = array_map(function($value) use ($type) {                                    
                                    settype($value, $type[0]);
                                    return $value;
                                }, $stringArray);
                                $query->whereIn($search_property, $numberArray);
                                static::$searchLog[] = [$search_name, "IN", $value];
                                break;
                            }
        ...
        if ($sort) {
            if (array_key_exists($sort, $extra_col)) {
                ...
                if (array_key_exists($sort, $this->selectHelper)) {
                    $parts = explode('.', $this->selectHelper[$sort]);
        ...
        $names =
            ...
            ->filter(function ($rec, $k) use ($table) {
                ...
                if ($rec instanceof Expression) {
                    if (method_exists(DB::connection('mongodb'), 'getQueryGrammar')) {
                        $qexpressionValue = $rec->getValue(DB::connection('mongodb')->getQueryGrammar());
                    } else {
            ...
            ->map(function ($rec) {
                if ($rec instanceof Expression) {
                    if (method_exists(DB::connection('mongodb'), 'getQueryGrammar')) {
                        $rec = $rec->getValue(DB::connection('mongodb')->getQueryGrammar());
                    } else {
